...
| -------- | -- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- | ------- |
| **P0**   | 1  | **Fix CI private repo auth** — replace `secrets.GITHUB_TOKEN` with a PAT (`secrets.PRIVATE_REPO_TOKEN`) or SSH deploy key. Verify by pushing and watching CI. ✅ done — workflow updated to use PRIVATE_REPO_TOKEN (2026-08-08) | CI is likely BROKEN right now                     | Low     |
| **P0**   | 2  | **Create `go.work`** for local cross-repo dev — `go 1.26.5\nuse .\nuse ../go-finding` (gitignored or committed depending on team preference). ✅ done — created 2026-08-08 (gitignored)                                         | Local dev workflow is broken                      | Low     |
| **P0**   | 3  | **Run `nix run .#lint`** and fix any findings — the canonical lint was never run this session. ✅ done — 0 issues                                                                                                               | Unknown lint state                                | Low     |
| **P0**   | 4  | **Run `nix run .#test-race`** — race detector was never run this session. ✅ done — clean                                                                                                                                       | Unknown race state                                | Low     |
...
| **P2**   | 12 | **Update FEATURES.md** — go-finding is now resolved from VCS, not a local replace. ✅ done                                                                                                                                      | Doc accuracy                                      | Trivial |
| **P2**   | 13 | **Add a CI badge** to README.md showing build status. ✅ done — added 2026-08-08                                                                                                                                                | Visibility                                        | Trivial |
| **P2**   | 14 | **Document the SSH `insteadOf` workaround** in CONTRIBUTING.md for new contributors with read-only global git config. ✅ done                                                                                                   | Onboarding                                        | Low     |
...
| **P3**   | 17 | **Add `go mod tidy` check to CI** — `git diff --exit-code go.mod go.sum` after tidy. ✅ done — added 2026-08-08                                                                                                                 | Prevent pseudo-version regression                 | Low     |
...
| **P3**   | 22 | **Add `nix run .#coverage`** to CI — generate coverage reports. ✅ done — coverage gate added (fail if < 90%)                                                                                                                   | Test visibility                                   | Low     |
| **P4**   | 23 | **Write integration test** that imports go-linter-sdk as an external module (simulating a consumer) to catch importability regressions. ✅ done — examples_integration_test.go                                                  | Regression prevention                             | Medium  |
| **P4**   | 24 | **Add `CODEOWNERS` file** for dependency files (go.mod, go.sum, flake.lock). ✅ done — created 2026-08-08                                                                                                                       | Review routing                                    | Trivial |
