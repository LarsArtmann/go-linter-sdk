...
| 4 | Wire `dprint.json` into the toolchain or remove it                             | Medium   | 15m    | TODO    | `dprint.json` (tracked since `4f4aee0`) formats md/json/yaml/dockerfile, but nothing in `flake.nix`, CI, or `.buildflow.yml` invokes it — orphan config; `nix fmt` (treefmt) covers only Go + Nix.                               |
